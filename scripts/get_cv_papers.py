...
def process_paper(paper, glm_helper, target_date):
    ...
    try:
        ...
        authors_str = ', '.join(authors[:8] if len(authors) > 8 else authors)  # 限制作者显示数量
        ...
        # 获取PDF链接
        pdf_url = next((link.href for link in paper.links if link.title == "pdf"), None)
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            title_cn_future = executor.submit(glm_helper.translate_title, title)
...
def get_cv_papers():
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            # 创建批处理进度条
            batch_pbar = tqdm(total=batch_size, desc="当前批次", unit="篇", leave=False)
            ...
            for paper in client.results(search):
                ...
                # 提交处理任务
                future = executor.submit(process_paper, paper, glm_helper, target_date)
        ...
        for category, papers in sorted_categories:
            ...
            print(f"{category:15s}: {len(papers):3d} 篇 (🆕 {num_new:3d} 新发布, 📝 {num_updated:3d} 更新)")
        print(f"{'='*30}")
        print(f"总计: {total_papers} 篇")
        
    except Exception as e:
        print(f"处理CV论文时出错: {str(e)}")
